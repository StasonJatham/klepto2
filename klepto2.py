...
class SkopeoClient:
    ...
    def pull_image(image_name: str, dest_path: str) -> bool:
        ...
        cmd = [
            "skopeo", "copy",
            "--override-os", "linux", # Ensure we get linux images
            "--retry-times", "3", # Retry on failure
            f"docker://{image_name}",
            f"dir:{dest_path}"
        ]
        try:
                 ...
                 for root, dirs, files in os.walk(directory):
                    if ".git" in dirs:
                        ...
                        if os.path.exists(config_file):
                            self.add_finding(f"Found GIT CONFIG: {git_dir}", console=True, level="WARNING")
                    ...
                    for filename in files:
                        if self._match_pattern(filename, pattern):
                            ...
                            if action_type == "shadow":
                                ...
                                self.add_finding(f"Found {title}: {filepath}", console=True, level="WARNING")
    ...
    def _check_shadow(self, filepath: str):
        try:
            with open(filepath, 'r', errors='ignore') as f:
                content = f.read()
                if '$' in content:
                    self.add_finding(f"Found SHADOW file: {filepath}", console=True, level="WARNING")
                    for line in content.splitlines():
                        if '$' in line:
                            self.add_finding(f"Shadow content: {line}", console=True, level="WARNING")
        except Exception as e:
            logger.error(f"Error reading {filepath}: {e}")

    def _cat_file(self, filepath: str):
        self.add_finding(f"Found sensitive file: {filepath}", console=True, level="WARNING")
        try:
            with open(filepath, 'r', errors='ignore') as f:
...
class Klepto2:
    ...
    def extract_image(self, image_dir: str, extract_root: str):
        ...
        if not files_to_extract:
            files_to_extract = [f for f in os.listdir(image_dir) if f not in ['manifest.json', 'version']]
    ...
    def process_image(self, image_name: str) -> Optional[str]:
        ...
        if ":" not in image_name:
            ...
            is_custom_registry = len(image_name.split('/')) > 1 and '.' in image_name.split('/')[0]
        ...
        if metadata:
            ...
            logger.info(f"Image Metadata: Created={created}, Arch={architecture}, OS={os_name}, Layers={layers_count}")
            if env_vars:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.info(f"Scan complete for {image_name}. Results saved to {output_file}")
            return output_file
    ...
    def run(self, inputs: List[str], mode: str):
        ...
        if mode == 'search':
            ...
            for term in inputs:
                ...
                is_image = ':' in term or (len(term.split('/')) > 1 and '.' in term.split('/')[0])
        ...
        logger.info(f"Identified {len(image_names)} unique images to scan: {image_names}")
        
        if not image_names:
            logger.info("No images to process.")
            return

        logger.info(f"Starting scan of {len(image_names)} images with {Config.MAX_WORKERS} workers.")
        
        with ThreadPoolExecutor(max_workers=Config.MAX_WORKERS) as executor:
            future_to_image = {executor.submit(self.process_image, img): img for img in image_names}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Klepto2: Scan Docker images for secrets.")
    ...
    parser.add_argument("--quiet", "-q", action="store_true", help="Suppress console output (only JSON output)")
